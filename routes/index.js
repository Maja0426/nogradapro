...
var systemMail = require('./../email/systemMail');
  ...
  if (req.body.password === req.body.samePassword) {
    ...
    User.register(newUser, req.body.password, function(err, regUser) {
      if (err) {
        req.flash(
          'error',
          'Valami hiba történt, valószínű már regisztrált email cím!'
        );
        ...
        passport.authenticate('local')(req, res, function() {
          req.flash(
            'success',
            'Üdvözlet a Nógrád Aprón ' + regUser.username + '.'
          );
          res.redirect('/ads');
        });
      }
      systemMail(
        `A Nógrádapró hirdetési oldalra új felhasználó regisztrált.\n\nFelhasználónév: ${
          regUser.username
    ...
    [
        ...
        User.findOne(
          ...
          function(err, user) {
            if (!user) {
              req.flash(
                'error',
                'Ehhez az email címhez nem tartozik felhasználó.'
              );
      ...
      function(token, user, done) {
        ...
        var mailOptions = {
          ...
          text:
            'Azért kaptad ezt a levelet mert a Nógrád Apró oldalon jelezted, hogy elfelejtetted a felhasználói jelszavad.\n\n' +
        ...
        smtpTransport.sendMail(mailOptions, function(err) {
          ...
          req.flash(
            'success',
            'Email került elküldésre a(z) ' +
              user.email +
              ' címre, a szükséges utasításokkal.'
          );
          done(err, 'done');
        });
      }
  ...
  User.findOne(
    ...
    function(err, user) {
      if (!user) {
        req.flash(
          'error',
          'A jelszóvisszaállító kulcs nem érvényes vagy lejárt.'
        );
        return res.redirect('/forgot');
      }
      res.render('reset', {
    ...
    [
      ...
      function(user, done) {
        ...
        var mailOptions = {
          ...
          text:
            'Szia,\n\n' +
            'Ez egy megerősítő levél arról, hogy a(z) ' +
            user.email +
            ' emailcímhez tartozó jelszavad megváltoztatásra került.\n'
        };
        smtpTransport.sendMail(mailOptions, function(err) {
          req.flash(
            'success',
            'Rendben! A jelszó megváltoztatása sikeres volt.'
          );
          done(err);
        });
      }
