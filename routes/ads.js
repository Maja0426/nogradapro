...
router.get('/mainCategory/:id', function(req, res) {
  ...
  var pageNumber = pageQuery ? pageQuery : 1;
  var noMatch = null;
  Ads.find({ mainCategory: req.params.id })
    .sort({ createdAt: -1 })
    .skip(perPage * pageNumber - perPage)
    .limit(perPage)
    .exec(function(err, allAds) {
      Ads.countDocuments({ mainCategory: req.params.id }).exec(function(
        err,
        count
      ) {
        if (err) {
...
router.get('/category/:id', function(req, res) {
  ...
  Ads.find({
    ...
    .limit(perPage)
    .exec(function(err, allAds) {
      Ads.countDocuments({ category: req.params.id }).exec(function(
        err,
        count
      ) {
        if (err) {
...
// UPDATE PAGE
router.put('/:id', middleware.checkUser, upload.single('image'), function(
  req,
  res
) {
  req.body.ads.description = req.sanitize(req.body.ads.description);
  ...
  if (req.file) {
    cloudinary.uploader.upload(req.file.path, function(result) {
      // add cloudinary url for the image to the ads object under image property
      req.body.ads.image = result.secure_url;
      Ads.findByIdAndUpdate(req.params.id, req.body.ads, function(
        err,
        updatedAd
      ) {
        if (err) {
